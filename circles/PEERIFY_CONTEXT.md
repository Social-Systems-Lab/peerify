...
32. **`git cherry -v`/ancestry-based promotion auditing is unreliable for this repo's actual workflow (confirmed 2026-09-05).** Cherry-picking mints new commit hashes even for identical content, so `git cherry -v`/`git log branch..branch` can report a commit as "still missing" from a target branch when its content is already there under a different hash — confirmed concretely when 7 already-shipped commits were mistakenly re-attempted, driving a cherry-pick into empty-commit and merge-conflict failures. **Going forward, scope every promotion by direct file-diff comparison** (`git diff branch..branch --stat`, narrowed to the feature's files) rather than trusting commit-ancestry tools as the source of truth for "what's actually missing." When individual commits in a series substantially rewrite what earlier commits in the same series just created, prefer a direct `git checkout branch -- <files>` (or a single squashed commit) over a commit-by-commit replay. See `SESSION_LOG.md`, 2026-09-05 entry, and the 2026-08-13 entry for an earlier promotion that also relied on `git cherry -v` (worked out that time, but the same false-negative risk applied).
33. **Venue/business participation in Offers — not yet scoped (2026-09-05).** Venues (`circleType: "circle"`) cannot create or show Offers today: the Presence-settings offerings editor and the offer-pin map query are both hardcoded to `circleType: "user"` only. Real feature decision needed before building: editor UI for venues, whether the offer-pin query should expand to include `circleType: "circle"`, and whether `offersVisible` should gate venue offers the same way it gates personal-profile offers. See `SESSION_LOG.md`, 2026-09-05 entry.
