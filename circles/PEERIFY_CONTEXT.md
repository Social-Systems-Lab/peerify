...
- **Song ovations ("claps"), Phase 2a** (promoted to prod 2026-08-18, commit `11d403f5`): a repeatable, uncapped tap on a track — upserts and `$inc`s a `Reaction` doc (`contentType: "track"`, `reactionType: "ovation"`) per tap. Deliberately no numbers shown to the tapping fan, ever — feedback is a burst/pulse animation (reusing `LikeButton`'s animation base), a persistent size/fill change once tapped (restored correctly on return visits, not just for the tapping session), and a short synthesized filtered-noise-burst tick sound (no audio asset committed) — gesture-unlocked inside the tap handler itself for iOS Safari's `AudioContext` policy. A one-time coach-mark tooltip explains the gesture on first encounter (localStorage-gated, same fail-open convention used elsewhere in this app). The aggregate ovation count is visible **only** to the track's owning artist/circle admin (gated server-side on `features.music.manage`, computed on demand from `Reactions` — never denormalized onto the public `Track` document, never included in the map-popup's public track payload). **Ovation notifications (Phase 2b) are deliberately deferred** — not built yet, intentionally, until real clap volume on prod gives a real basis for a digest cadence instead of guessing blind.
- **Primary Genre** (2026-07-12): a `primaryGenres: string[]` field on artist profiles (max 3, plus an "Other" option with a free-text `primaryGenreOther`), set via a checkbox group on Artist Identity settings and displayed as badges on the public profile ("Other (custom-genre)" formatting when free-text is used). Synced to the top-level `Circle` document (same pattern as the old `causes`/SDG field it replaces) so search filtering runs server-side via Mongo `$in` queries, not client-side. Surfaced in advanced search as a multi-select filter with removable pills, a filter-count badge, and a "Press Enter to apply" hint. Also wired into Qdrant vector search. **Known gap, corrected 2026-09-06:** this note previously said the genre filter does not affect Events at all. Investigated while building the 2026-09-06 Advanced Filters work: Events actually do inherit genre from their host circle server-side (`getOpenEventsForMap`, `event.ts`) and the genre param is already passed to the events fetch regardless of category — a real, functioning filter, not a gap. Venues, however, are confirmed to never carry `primaryGenres` at all (no venue genre UI exists anywhere), so a Venues-only genre filter would always return zero results. See §00 carry-forward and §6.7, and `SESSION_LOG.md`'s 2026-09-06 entry.
...
- Circles, messaging, reviews, noticeboards — inherited Circles primitives, present but not all Peerify-tailored yet.
- **Events module — substantially Peerify-tailored as of 2026-08-20**, no longer just the inherited Circles primitive. Draft → review → publish flow with a visible draft-status badge; multi-artist support (additional bands with delegated edit access); optional "Ticketed event" toggle (informational Price/Currency/Payment-info only — no real payment processing yet, see §00 payments milestone); a "private/home event" toggle (hides exact venue, approximate-area-only map placement, approval-required access); progressive-disclosure create form (Images/Artists/Virtual/Capacity/Recurring collapsed under "More options", Visibility/Noticeboard-sharing always visible); a "preview as a fan would see it" link using the real anonymous-visitor sanitizer; virtual events correctly surfaced on the map feed. An event's host circle is no longer fixed at creation — the creator can change it afterward (instant if they administer the target circle, otherwise a pending request the target circle's admins approve), with tasks/comment-feed migrating to the new host and stale URLs redirecting. Personal (non-artist/venue) profiles now get the Events module enabled by default like venues already did. Full detail: `SESSION_LOG.md`, 2026-08-14 (multi-artist) and 2026-08-20 (everything else above) entries.
...
- **Venue event-tags gating fix — as of 2026-08-29.** The Default event tags settings card (and its 3 other consumers) was rendering/applying for every circle type instead of Venue-only; now correctly gated everywhere. Full detail: `SESSION_LOG.md`, 2026-08-29 entry.
- **Second "Save Changes" button — as of 2026-08-29.** Added below the Default event tags section on the About settings form, Venue circles only — the one circle type where the form is now long enough to need it. Full detail: `SESSION_LOG.md`, 2026-08-29 entry.
- **Artist type field simplification + rename — as of 2026-08-29.** Signup's "Band name" field renamed to "Performing name"; Artist Identity's "Artist types" checkbox list simplified from 9 options to 4 (Solo artist, Band, DJ, Other), with "Other" revealing a multi-tag free-text chip input. Legacy values migrated on both staging and prod. Full detail: `SESSION_LOG.md`, 2026-08-29 entry.
- **Artist settings page cleanup — as of 2026-08-30 (staging only, not yet promoted to main/prod).** Published status card now hides once published (confirmed no unpublish path exists anywhere); a second Save Changes button added after Genre selection; Booking enquiries split into its own collapsible card; Spotify added to Music Links (settings + public profile icon row); "Looking for / Open to" hidden (confirmed dead code, no public-facing counterpart); "Publish Circle" → "Publish Profile" copy for managed identities only (plain Circles/Projects keep "Circle"). Full detail: `SESSION_LOG.md`, 2026-08-30 entry.
- **Pledge dialog overhaul — as of 2026-08-30/31.** Willingness-to-help (renamed "Contribute to tour") collapsed by default; currency badge now defaults to EUR when unset, with its option list extracted into a shared `PEERIFY_CURRENCY_OPTIONS` constant; "Event type" investigated then removed entirely (confirmed both downstream readers already degrade gracefully); a Join Crew button added to the dialog (reusing the existing Crew flow) and moved to its own row below the note field; Pledge buttons made consistently orange (`#FE801B`) and Crew buttons made consistently solid dark-ink (`#1A1612`) across every real surface; "Space for 20-30 people" checkbox replaced with a conditional free-text "Approximate capacity" field under "Host." Full detail: `SESSION_LOG.md`, 2026-08-30/31 entry.
- **Multi-pledge upsert fix + data reconciliation — as of 2026-08-31.** Repeat pledges from the same fan to the same artist previously created duplicate records with no dedup; `createPeerifyPledge` now upserts on `(artistCircleId, pledgerDid)`, backed by a DB-level unique index, and the dialog pre-fills from an existing pledge with an "update" framing. Existing duplicates reconciled on both staging and prod (prod: 16 → 8 records, every group reviewed and approved before any write). Cherry-picked to `main` as a 6-commit chain together with the Pledge dialog overhaul above; deployed and verified live on prod. Full detail: `SESSION_LOG.md`, 2026-08-31 entry.
- **Location/Crew-Offers privacy fix — as of 2026-09-05.** `filterLocations` made non-mutating and viewer-aware, then wired into every read path that returned a circle's/member's location with no gating at all (`getMembers`/`getCrewMembers`, every author lookup in `feed.ts`) — previously not even the existing `mapVisible`/`searchable` opt-in applied there. Crew Offers (`tourTeamOfferings`) binary-gated out of any response reachable by a non-crew, non-owner viewer. Explicit ownership re-check added to the About/Presence settings pages. Full detail: `SESSION_LOG.md`, 2026-09-05 entry.
- **Offers map feature — as of 2026-09-05.** New "Offers" pill on `/explore` — anonymous, per-offer map pins (one pin per individual offer, not per person; no name/photo/handle/profile-link anywhere in the feature) for accommodation/transport/meal/sound/custom offers of help, browsable before any Crew/artist relationship exists. Same-coordinate offers spread apart via deterministic jitter. Visibility gated by a new field, `offersVisible`, fully independent of `mapVisible`/`searchable` — defaults `true` for newly-created circles only, never retroactive. Pin location decoupled from the profile's own location-precision setting: real coordinate only if precision is already "Exact" (the venue case), otherwise a fixed coarse (~1km) fallback so a pin always renders. **Venue gap closed 2026-09-06** — see the new bullets below (carry-forward item 33 resolved). Full detail: `SESSION_LOG.md`, 2026-09-05 entry.
- **Venue participation in Offers — as of 2026-09-06.** Venues (`circleType: "circle"`, `identityType: "venue"`) can now create offers from Presence settings, restricted to a curated subset (hosting a show, sound & equipment help, meal, custom) — accommodation/transport/city-guide read as personal-hospitality gestures that don't fit a business profile, so they're excluded for venues. `getOfferMapPins` widened to include venue rows (bands still out of scope). Venue offer pins carry real identity (name, "View profile" link, picture) in the hover popup and click-preview — but **not** on the marker face itself, which always shows the per-offer-type icon regardless of identity, so offer types stay distinguishable at a glance. Also fixed: a venue's stored `location.precision` was being silently clamped to city-level unless its separate public-address-visibility setting was `"public"`; decoupled so precision is stored exactly as chosen, with the public-display ceiling enforced separately at read time (`redactCircleLocationForViewer`, `lib/utils.ts`, used by `getSwipeCircles`/`search.ts`/`feed.ts`). Full detail: `SESSION_LOG.md`, 2026-09-06 entry.
- **Venue offer-pin jitter/grouping fix — as of 2026-09-06.** Venue-identity offer pins excluded from the same-coordinate jitter (built for anonymous pins, which have no fixed reference point to look inconsistent against — a venue's pins do, since the venue has its own separate profile pin at the true coordinate). Co-located same-venue offerings now merge into one marker with a count badge instead of stacking on top of each other; click-preview lists every offer type when grouped. Full detail: `SESSION_LOG.md`, 2026-09-06 entry.
- **Advanced Filters: per-offer-type filter + category-aware section visibility — as of 2026-09-06.** New offer-type filter modeled on the existing Genre picker. Each Advanced Filters section now only shows when relevant to the selected category: Genre for Artists or Events (Events genuinely inherit genre from their host circle — see the Primary Genre correction above), Physical-events-only for Events, Calendar for Artists/Venues/Events/Offers (effectively always). Checking "Offers" nudges "Artists" off by default (reversible, one-directional). Full detail: `SESSION_LOG.md`, 2026-09-06 entry.
...
**✅ RESOLVED (2026-07-09) — second staging→main promotion complete.** Merged staging into main (merge commit `1cd9f105`, pushed), covering the full `isVerified` auto-verify overhaul (items 16/17 below) plus the same day's four follow-up fixes (check-email popup simplification, unified red-text banners, Forum nested-reply gating fix, Unverified-pill investigation). Built in an isolated detached worktree first to verify the build without touching the live PM2 process, then ran `scripts/deploy-peerify.sh` for the real deploy — PM2 restarted cleanly (0 restarts, new pid), staging untouched throughout. Verified live on peerify.one via HTTP + content checks. Prod, staging, and main are now in sync at `1cd9f105`. See 2026-07-09 (cont. #2) `SESSION_LOG.md` entry.
...
11. **`isVerified` overloaded for map/search discoverability** (discovered 2026-07-05). `isVerified` is meant to represent "account approved/activated by an admin" (set via `activateUserAccount()`, triggered by the admin "Verify Account" action or approving a verification request). However, this same flag is also the sole gate determining whether a personal/individual (`circleType: "user"`) profile appears in map results and search/discovery (see `isDiscoverableCircle`, `src/lib/data/search.ts:46-47`, and the identical `$or` clause in `getSwipeCircles`, `circle.ts:173`). This means any fan account an admin approves automatically becomes map-visible, regardless of `isPublic`, location precision, or explicit user choice. `isPublic` does not gate discovery at all currently — it only affects follow-approval requirements. Confirmed via prod DB query 2026-07-05: 5 of 9 `circleType: "user"` docs have `isVerified: true` (all pre-launch test/dev accounts, not real users), and all 5 have location coordinates set, so all 5 currently produce map pins. Not urgent pre-launch (no real users onboarded yet), but must be fixed before onboarding real fans, since it silently violates the stated MVP principle that only Artists/Venues/Events should be discoverable on the map for now. Proposed direction for a future session: decouple discovery visibility from `isVerified` into its own explicit field, and design a real visibility/discoverability setting individual users can control themselves (tie-in with the Access & Permissions card work from the 2026-07-05 Settings cleanup session). **⚠️ Escalated 2026-07-08:** `isVerified` for personal profiles now sets automatically as soon as a fan adds a profile picture + About text (see item 16) — no admin gate at all anymore. This makes the map/search leak in this item far more likely to trigger for real users very soon after onboarding (previously only 5 pre-launch test accounts had `isVerified: true` via manual admin action; going forward, most real fans who complete their profile will trip it within their first session). Fixing this decoupling is now materially more urgent, not just a pre-launch nice-to-have.
16. **isVerified auto-verify overhaul for personal profiles (2026-07-08; ✅ promoted to main/prod 2026-07-09).** Replaced the admin-approval-gated `isVerified`/`accountStatus` flow with an automatic one, personal (`circleType: "user"`) profiles only — artist/venue verification untouched. `updateCircle()` (`src/lib/data/circle.ts`) now auto-verifies a personal profile the moment it has both a custom profile picture and non-empty About text (reusing the existing `getVerificationReadiness()` checklist), forward-only (never revokes on later edits). This directly unblocks posting/commenting/messaging, which are gated on `isVerified` via `needsToBeVerified` (previously required an admin to manually approve every fan before they could interact with anything). Also: hid the manual "Request Verification" UI for personal profiles specifically (`VerifyAccountButton`, `VerificationSettingsCard` — commented out, not deleted; artist/venue verification queue/UI untouched); added a shared explainer banner (`UNVERIFIED_PROFILE_EXPLAINER`) wherever posting/commenting/chat is blocked for an unverified user; added a one-time "profile complete" notification on the auto-verify transition (reuses the existing `sendUserVerifiedNotification`, new optional `messageBody` param); fixed a duplicate "Save Changes" button on the personal-profile About Settings page (a shared `renderSaveButton()` section-checkpoint helper was firing unconditionally in 3 places that render adjacent to each other for personal profiles specifically). Committed to `staging` (`08dc5fd7`) and pushed. **✅ Item 17's open issues investigated and resolved 2026-07-09** (commit `b072270a`, staging, pushed + deployed). **✅ Promoted to `main` and deployed to prod 2026-07-09** (merge commit `1cd9f105`) along with the same day's four follow-up fixes — see the promotion note above and the 2026-07-09 (cont. #2) `SESSION_LOG.md` entry. No longer staging-only.
17. **✅ RESOLVED 2026-07-09 — open issues from 2026-07-08 manual browser testing of the item-16 work.** Investigated all four; root cause for the two "not visible"/"not working" reports was **test-account state, not a rendering bug**: a live staging-DB query found 7 of 8 personal test accounts had already auto-verified (same-day timestamps as the 07-08 testing session) — the accounts were verified *before* the blocked-state UI was actually exercised, since picture+about had been saved first to set up a realistic test profile. Real bugs found and fixed along the way (commit `b072270a`):
    - **Stale "Request Verification" welcome-popup copy** — lived in `src/components/modules/home/home-content.tsx` (the end-of-signup `Dialog`, personal-profiles-only via the `isOwnUserProfile`/`circleType === "user"` guard). Replaced with copy describing the new auto-verify unlock mechanic. Confirmed this popup is not shared with artist/band/venue onboarding (same `HomeContent` component, but the dialog itself only ever opens for `circleType: "user"`).
    - **Real bug: mission/description write-path mismatch.** The Home tab's inline "click-to-edit" About field (`home-content.tsx`) wrote to `circle.mission` instead of `circle.description` whenever `description` was empty — but `getVerificationReadiness()`/`hasAboutText()` (`src/lib/verification-readiness.ts`) only ever reads `description`/`content`, never `mission`. A user completing their About text via that inline field (rather than Settings) could have visibly-saved text that silently never counted toward auto-verification. Fixed by always writing `description` for personal profiles specifically (`id={isUser || circle.description ? "description" : "mission"}`) — scoped to `isUser` only after an audit confirmed `mission` is still a live, separately-displayed field for regular community/org circles (Settings input + AboutPage "Mission" section + swipe-card/content-preview quote boxes), so an unscoped fix would have silently orphaned their mission content.
    - **Banner consistency cleanup:** `post-form.tsx` was checking a raw `!user.isVerified` instead of the shared `canPerformRestrictedAction()`/`isVerifiedUser()` helpers used everywhere else (skipped the admin bypass; latent inconsistency, not confirmed as live-exploitable). `CommentSection.tsx`'s nested reply composer had no verification banner at all (only the top-level new-comment composer did) — added one.
    - **Admin-messaging check** (`getRestrictedActionMessage("contact circle admins")` in `mongo-actions.ts`) was confirmed correctly wired to the same `isVerified`/`verificationStatus` vocabulary as posting/commenting, with a fresh DB read (not stale session/JWT data) and checking the sender's own DID (not the admin's) — the "still blocked" report was a test-sequencing artifact, not a code bug, though it remains an architecturally separate code path from `isAuthorized()`/`needsToBeVerified` worth eventually consolidating.
    - **Verification method:** full browser E2E (Playwright) was attempted but blocked by missing system shared libraries for headless Chromium (`libnspr4.so` etc., needs `sudo apt`) — declined rather than install. Fell back to exercising the real production functions (`updateCircle()`, `isAuthorized()`, `canPerformRestrictedAction()`, `getRestrictedActionMessage()`) directly against a throwaway account on the staging DB, mimicking exact real-signup defaults: confirmed a genuinely fresh/unverified account is blocked from posting/commenting/admin-contact; reproduced the mission-field bug live (writing About text to `mission` does NOT auto-verify even with a picture set); confirmed the fix (writing to `description`) DOES auto-verify and unblocks all three gates. Throwaway doc deleted after.
...
20. **"Instruments played" field — design decision locked 2026-08-29, not yet built.** Strategy discussion concluded this will NOT be a standalone profile field; it will live inside a future "Open to collaboration" module, alongside skill level, demo track, and collaboration types. Not yet scoped: the collaboration-type taxonomy, whether search is browse-only or also supports posted requests, and the shape of the location/availability field. No code changes yet — flagging here so it isn't independently designed as a standalone field before the Collaboration module itself is scoped.
21. **Performer/Teacher signup strategy — decided 2026-08-29, not yet built.** "Performer" will be a category flag on the existing Artist role (not a new backend role), with Crew treated as a togglable per-circle setting rather than type-locked. "Teacher" is broadened beyond "music teacher" to cultural/performance teaching generally, and remains a genuinely separate, lightweight role from Artist. Cross-reference item 2 above (`PEERIFY_DEFAULT_SONGWRITER_AVATAR_URL`, still unbuilt) — the 2026-08-28 Songwriter discussion confirmed that item is still pending, and Songwriter should stay architecturally separate from the Collaboration module described in item 20.
22. **Pledge Dashboard clarity + manual pledger messaging (Tier 2, not yet scoped, 2026-08-31).** Messaging ties directly to the existing unbuilt pledge-campaign group-formation-at-10%-threshold mechanic already described in §3.4/§6.6 — a manual version (the artist/admin messaging pledgers by hand) should be built first as a simpler precursor, before any automated group-formation flow.
23. **Distance-tiered pledge needs (Tier 3, not yet scoped, 2026-08-31, reaffirmed 2026-09-01).** Explicitly flagged as needing a combined design session with the already-deferred location-based/tiered booking fees item (item 14 above) — not to be scoped independently of it.
24. **"My Pledges" personal inventory page for fans (not yet scoped, 2026-08-31, reaffirmed 2026-09-01).** A page listing a fan's own pledges across artists — comparable in spirit to the artist-facing Pledge Dashboard, but for the fan's own side. Phased: build a flat list first; richer features (editing, status tracking, etc.) only if usage justifies it.
25. **Currency conversion / blended cross-currency totals (deferred, 2026-09-01).** The currency coherence pass (fan-selected pledge currency, per-currency subtotals and per-row display) deliberately stopped short of real exchange-rate conversion or a single blended total across currencies — both explicitly out of scope for that work, not yet scoped as their own feature.
26. **Unified Explore-style map showing pledges + Crew together (deferred, 2026-09-01).** Mentioned alongside the Crew Dashboard v1 build — that build deliberately shipped without a map view (same reasoning as the Pledge Dashboard's own map deferral); a combined pledges+Crew map view remains a future possibility, not yet scoped.
27. **Investigate the staging→main cherry-pick backlog (2026-09-03, predates this session).** A number of commits appear to exist on `staging` that were never cherry-picked to `main` — this predates tonight's work, not something introduced by it. Needs reconciling: confirm whether `main`/prod is actually missing real shipped features, or whether equivalent work already landed on `main` under different commit hashes from earlier cherry-picks (content-identical but different SHAs is the norm for this workflow — every commit promoted tonight did exactly that, e.g. staging's `2cf7fba0` ↔ main's `a939aac5`). **Concrete instance found while cherry-picking this very doc commit (`68378497`) to `main`:** this file's two branches had independently appended carry-forward items 18-21 with completely different content and no shared ancestor context (resolved above by merging both sets and renumbering 18-30, deduping items 23/24 which turned out to be the same two items restated a day apart); `SESSION_LOG.md` similarly turned out to be missing the 2026-09-01 and 2026-09-02 entries on `main` (see that file's new 2026-09-03 entry for detail), and has a pre-existing, unrelated chronological-ordering problem further down. Not yet investigated for the *commit* backlog specifically (this only covered the two docs files); don't assume either direction without checking.
28. **Small cleanup: contact form reason dropdown logs a React "controlled to uncontrolled" console warning (2026-09-03).** `peerify-contact-dialog.tsx:44` — `defaultValues` sets `reason: undefined`, so the `Select`'s `value={field.value}` starts undefined and only becomes a string once the user picks one; React warns on that transition. Likely fix is defaulting to `""` instead of `undefined`, but not yet made — flagged, not fixed.
29. **Small cleanup: `/about` page 404s, including on client-side prefetch (`_rsc` request) (2026-09-03).** The landing-page footer links to `/about` (`peerify-landing-page.tsx:380`), but there is no `/about` route anywhere under `src/app` — confirmed via search, not just observed as a symptom. A full navigation would 404 outright; Next.js `Link` prefetching surfaces the same 404 earlier, on scroll-into-view, as an `_rsc` request. Investigate what the page should actually contain before adding the route — not yet fixed.
30. **Venue/host signup flow remains unscoped (2026-09-03).** "List a venue or space" (CTA row) and the "For Hosts & Venues" Roles-section card are intentionally hidden behind `SHOW_VENUE_SIGNUP = false` (`peerify-landing-page.tsx`, commit `3fbd7b4f`/`a9d582d0`) — `/signup/pilot` has no venue option, no dedicated route, and no `CreatePeerifyVenueDialog` wiring from the landing page. Re-enabling these needs that flow designed and built first, not just the flag flipped.
31. **`savePresence` missing an authorization check for non-user circles (found 2026-09-05).** The Presence settings save action never calls `isAuthorized` for non-user (band/org) circles — personal (`circleType: "user"`) profiles are already protected via `updateCircle`'s own ownership check, but a band/org circle's presence data (offers/needs/engagements text) may currently be writable by any authenticated user who knows its circle ID. Needs its own scoped fix. See `SESSION_LOG.md`, 2026-09-05 entry.
32. **`git cherry -v`/ancestry-based promotion auditing is unreliable for this repo's actual workflow (confirmed 2026-09-05, reconfirmed 2026-09-06).** Cherry-picking mints new commit hashes even for identical content, so `git cherry -v`/`git log branch..branch` can report a commit as "still missing" from a target branch when its content is already there under a different hash — confirmed concretely when 7 already-shipped commits were mistakenly re-attempted, driving a cherry-pick into empty-commit and merge-conflict failures. **Going forward, scope every promotion by direct file-diff comparison** (`git diff branch..branch --stat`, narrowed to the feature's files) rather than trusting commit-ancestry tools as the source of truth for "what's actually missing." When individual commits in a series substantially rewrite what earlier commits in the same series just created, prefer a direct `git checkout branch -- <files>` (or a single squashed commit) over a commit-by-commit replay. **2026-09-06: same false-signal pattern hit again** across three separate promotions the same night — `git log staging..main` and `git merge-tree` both reported real-looking main-only divergence/conflicts on files that turned out, on direct tip-to-tip diff inspection, to already be fully reconciled between the branches. Treat this as a standing property of the repo's history, not a one-off. See `SESSION_LOG.md`, 2026-09-05 and 2026-09-06 entries, and the 2026-08-13 entry for an earlier promotion that also relied on `git cherry -v` (worked out that time, but the same false-negative risk applied).
33. **✅ RESOLVED (2026-09-06) — Venue/business participation in Offers.** Venues (`circleType: "circle"`, `identityType: "venue"`) can now create offers from Presence settings (curated subset: hosting a show, sound & equipment help, meal, custom) and their offer pins show on the map with real identity (name, picture, profile link) via the hover popup/click-preview, per-offer-type icon on the marker face. Also fixed the venue location-precision clamp and the same-coordinate jitter/stacking bugs this surfaced. Bands remain out of scope, a separate future decision. See `SESSION_LOG.md`, 2026-09-06 entry.
34. **`getShareablePostPreview`/`buildPostDisplayPreview` has no location redaction at all (found 2026-09-06).** A post author's or a mentioned circle's exact location could leak via a shareable/public post preview — a different bug class from the venue-precision clamp fixed the same night (total absence of redaction here, not a coupling bug). Noticed while auditing `feed.ts`'s other redaction call sites; not fixed. See `SESSION_LOG.md`, 2026-09-06 entry.
35. **Venues have no `mapVisible`-equivalent general-visibility opt-in (found 2026-09-06).** Every published venue appears unconditionally on the general Explore map/search today, gated only by location precision (now correctly enforced per item 33's fix) — there's no all-or-nothing toggle the way personal profiles have via `mapVisible`. Worth a future conversation on whether venues need their own general-visibility control, separate from `offersVisible`. See `SESSION_LOG.md`, 2026-09-06 entry.
36. **Venue settings (About/Presence) needs a cleanup pass (flagged 2026-09-06, deliberately deferred).** No nav link to the Offers/Presence settings page for venues; "Use Current Location" button color is misleading; the Venue Identity card scope-creeps into Room & Capacity/Technical setup fields with no clear save boundary; Website/Instagram render as raw text instead of the social-link icon row other circle types get; unclear where a venue's Contact email actually surfaces publicly. Deliberately deferred to its own dedicated session, not touched 2026-09-06. See `SESSION_LOG.md`, 2026-09-06 entry.
...
**Genre-aware search — built 2026-07-12, still only half-covers the map.** The Primary Genre filter does real server-side filtering for Artists and Venues via Mongo `$in`, but Events have no genre field, so a genre filter can leave stray event pins visible with no matching artists/venues around them — still true as of the Category-aware search work below (`searchContentAction` only ever queries circles). See §00 carry-forward item 18 for the fix options under discussion. The rest of Advanced Filters — multi-select Category, Physical-events-only, Calendar — is now also built; see §0 and `SESSION_LOG.md` 2026-08-26/27.
