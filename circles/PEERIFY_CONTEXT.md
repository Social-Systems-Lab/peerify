...
16. **isVerified auto-verify overhaul for personal profiles (2026-07-08, staging only, not yet promoted to main).** Replaced the admin-approval-gated `isVerified`/`accountStatus` flow with an automatic one, personal (`circleType: "user"`) profiles only — artist/venue verification untouched. `updateCircle()` (`src/lib/data/circle.ts`) now auto-verifies a personal profile the moment it has both a custom profile picture and non-empty About text (reusing the existing `getVerificationReadiness()` checklist), forward-only (never revokes on later edits). This directly unblocks posting/commenting/messaging, which are gated on `isVerified` via `needsToBeVerified` (previously required an admin to manually approve every fan before they could interact with anything). Also: hid the manual "Request Verification" UI for personal profiles specifically (`VerifyAccountButton`, `VerificationSettingsCard` — commented out, not deleted; artist/venue verification queue/UI untouched); added a shared explainer banner (`UNVERIFIED_PROFILE_EXPLAINER`) wherever posting/commenting/chat is blocked for an unverified user; added a one-time "profile complete" notification on the auto-verify transition (reuses the existing `sendUserVerifiedNotification`, new optional `messageBody` param); fixed a duplicate "Save Changes" button on the personal-profile About Settings page (a shared `renderSaveButton()` section-checkpoint helper was firing unconditionally in 3 places that render adjacent to each other for personal profiles specifically). Committed to `staging` (`08dc5fd7`) and pushed; **not yet merged to `main`/deployed to prod** — see item 17 for open issues found in manual testing that should be resolved first.
17. **Open issues from 2026-07-08 manual browser testing of the item-16 work (staging) — investigate next session, not yet diagnosed:**
    - Stale "Request Verification" copy still visible somewhere in the signup flow — likely a leftover reference to the old manual-verification wording that needs updating/removing now that personal profiles auto-verify.
    - The new explainer banners (post/comment/chat) were not visible during manual testing despite being deployed — needs investigation (build/cache issue? wrong condition? banner logic not matching the account state actually exercised?).
    - Possible bug: an unverified user may still be able to comment when they shouldn't be — needs repro and root-cause (check `CommentSection.tsx`'s `canComment` computation and `client-auth.ts`'s `isAuthorized` against a real unverified test account).
    - An admin-messaging check was still blocked/incomplete during testing — needs follow-up to figure out what specifically didn't work.
