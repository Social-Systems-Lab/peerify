...
7. Create `deploy-staging.sh` — same build→copy→restart shape as `deploy-peerify.sh`, but pointed
   at the staging worktree/paths (`~/apps/peerify-staging/circles/circles`, standalone root
   `.next/standalone/apps/peerify-staging/circles/circles`), using `pm2 restart peerify-staging`
   (not delete/recreate, since the staging process persists across sessions). Currently staging
   deploys require manual copy of `.next/static` and `public/` — easy to forget, already cost one
   session (2026-07-03).
